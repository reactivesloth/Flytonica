...
namespace RTG
{
    ...
    {
        private static List<Transform> _transformsChildren = new List<Transform>();
        ...
        public static void GetAllChildrenAndSelf(this GameObject gameObject, List<GameObject> childrenAndSelf)
        {
            ...
            _transformsChildren.Clear();

            gameObject.GetComponentsInChildren<Transform>(_transformsChildren);
            foreach(var transform in _transformsChildren)
                childrenAndSelf.Add(transform.gameObject);
        }
