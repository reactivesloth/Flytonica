// --------------------------------------------------------------------------------------------------------------------
// <copyright file="UxrFishNetworkTransform.cs" company="VRMADA">
...
namespace FishNet.Component.Transforming
{
    ...
    public sealed class UxrFishNetworkTransform : NetworkBehaviour
    {
        #region Types.
        [System.Serializable]
        ...
        /// This is custom and usually does not exist in FishNet's Network Transform.
...
#pragma warning restore CS0414 //Not in use.
        ...
        private void SetDefaultGoalData()
        {
            ...
            void SetLastReceived(TransformData td)
            {
                ...
                if (useWorldSpace)
                {
                    td.Update(0, t.position, t.rotation, t.localScale, t.position, parentBehaviour);
                }
                else
                {
        ...
        private void SerializeChanged(ChangedDelta changed, PooledWriter writer, byte lodIndex)
        {
            ...
            if (_synchronizePosition)
            {
                ...
                {
                    original = useWorldSpace ? t.position.x : t.localPosition.x;
                ...
                {
                    original = useWorldSpace ? t.position.y : t.localPosition.y;
                    compressed = original * multiplier;
                    if (localPacking != AutoPackType.Unpacked && Math.Abs(compressed) <= maxValue)
                    {
                ...
                {
                    original = useWorldSpace ? t.position.z : t.localPosition.z;
                    compressed = original * multiplier;
                    if (localPacking != AutoPackType.Unpacked && Math.Abs(compressed) <= maxValue)
                    {
            ...
            if (_synchronizeRotation)
            {
                if (ChangedContains(changed, ChangedDelta.Rotation))
                {
                    ...
                    writer.WriteQuaternion(useWorldSpace ? t.rotation : t.localRotation, _packing.Rotation);
                }
            }
            ...
            if (ChangedContains(changed, ChangedDelta.Extended))
            {
                ...
                writer.FastInsertUInt8Unpacked((byte)flagsB, startIndexB);
            }

            //Insert flags.
            writer.FastInsertUInt8Unpacked((byte)flagsA, startIndexA);
        ...
        private void UpdateParentBehaviour()
        {
            ...
            if (parent == null)
            {
                ...
                if (_parentTransform != null && base.NetworkObject.RuntimeParentNetworkBehaviour != null)
            ...
            else
            {
                ...
                {
                    //Check for being set without using nob.SetParent.
                    if (base.NetworkObject.RuntimeParentNetworkBehaviour != parent)
        ...
        private void SendToClients(byte lodIndex)
        {
            ...
            else
            {
                ...
                for (int i = lodIndex; i >= 0; i--)
                {
                    ...
                    if (useWorldSpace)
                        lastSentData.Update(0, t.position, t.rotation, t.localScale, t.position, _parentBehaviour);
                    else
        ...
        private void SendToServer(TransformData lastSentTransformData)
        {
            ...
            * Tick doesn't need to be set for whoever controls transform. */
            Transform t = transform;
            if(useWorldSpace)
                lastSentTransformData.Update(0, t.position, t.rotation, t.localScale, t.position, _parentBehaviour);
            else
        ...
        private bool HasChanged(TransformData td)
        {
            ...
            if (useWorldSpace)
            {
                changed = (td.Position != transform.position ||
                                td.Rotation != transform.rotation ||
                                td.Scale != transform.localScale);
            }
            else
            {
                changed = (td.Position != transform.localPosition ||
        ...
        private ChangedDelta GetChanged(ref Vector3 lastPosition, ref Quaternion lastRotation, ref Vector3 lastScale, NetworkBehaviour lastParentBehaviour)
        {
            ...
            Vector3 position = useWorldSpace ? t.position : t.localPosition;
            ...
            Quaternion rotation = useWorldSpace ? t.rotation : t.localRotation;
        ...
        private void SnapProperties(TransformData transformData, bool force = false)
        {
            ...
            if (!useWorldSpace)
            {
                ...
                {
                    Vector3 position;
                    position.x = (force || _positionSnapping.X) ? transformData.Position.x : t.position.x;
                    position.y = (force || _positionSnapping.Y) ? transformData.Position.y : t.position.y;
                    position.z = (force || _positionSnapping.Z) ? transformData.Position.z : t.position.z;
                    t.position = position;
                }
                ...
                if (_synchronizeRotation)
                {
                    Vector3 eulers;
                    Vector3 goalEulers = transformData.Rotation.eulerAngles;
                    eulers.x = (force || _rotationSnapping.X) ? goalEulers.x : t.eulerAngles.x;
                    eulers.y = (force || _rotationSnapping.Y) ? goalEulers.y : t.eulerAngles.y;
                    eulers.z = (force || _rotationSnapping.Z) ? goalEulers.z : t.eulerAngles.z;
        ...
        private void SetCalculatedRates(byte lodIndex, TransformData prevTd, RateData prevRd, GoalData nextGd, ChangedFull changedFull, bool hasChanged, Channel channel, bool asServer)
        {
            ...
            if (ChangedFullContains(changedFull, ChangedFull.Position))
            {
                ...
                if (_enableTeleport)
                {
                    ...
                    if (_scaleThreshold)
                        tt *= Managing.Observing.ObserverManager.GetLevelOfDetailInterval(lodIndex);
                    //Over threshold.
                    if (distance >= tt)
                    {
        ...
        private void ObserversUpdateClientAuthoritativeTransform(ArraySegment<byte> data, Channel channel)
        {
            ...
            //Not new data.
            uint lastPacketTick = base.TimeManager.LastPacketTick.Value();
        ...
        private void UpdateTransformData(ArraySegment<byte> packetData, TransformData prevTransformData, TransformData nextTransformData, ref ChangedFull changedFull, out byte lodIndex)
        {
            ...
            nextTransformData.Tick = base.TimeManager.LastPacketTick.Value();
        }
...
#else
    public sealed class UxrFishNetworkTransform : MonoBehaviour
    {
    }
#endif
}